...
const uniq = (arr, callback) => {
  let modArray = arr.filter((item, index) => arr.indexOf(item) === index)
  callback(modArray)
}
...
uniq(names, (uniqArr) => {
 console.log(`The new names array with all the duplicate items remove is ${uniqArr}`)
})
...
const each = (arrOfNames, callback) => {
  for (let i = 0; i < arrOfNames.length; i++){
    // console.log(`${arrOfNames[i]}, ${i}`)
